...
**Source file:** `app/services/model_detection.py`
...
- Pleasantries and well-wishes: "Hope you're doing well", "Hope this finds you well", "Trust you are well", "Happy Friday"
...
- "Collective: We need to get this done" may be too aggressive — aspirational "we need to" in meetings often isn't a real commitment. Monitor false positive rate.
- `user_relationship` relies on the [User Identity Profile](../product/domain-policy.md#identity) system being populated. Until identity onboarding is complete, `mine` won't resolve correctly.
- No `speech_act` classification yet — requests and self-commitments land as the same object. WO-4 adds this.
